...
from .GemmaAgentVerifyier import gemma_agent_verifier
                ...
                if agent_prompt:
                    ...
                    try: 
                        ...
                        gemma_agent_verifier(prompt=f"""
                            You are Gemma Agent Verifier, an AI cybersecurity analyst responsible for validating and cross-checking information from multiple agent sources.
                            Your task is to examine the provided articles along with the outputs generated by the following agents:
                            Llama response: {response_text.get("llama_response")}
                            Agent Gemma response: {response_text.get("gemma_response")}
                            Agent Deepseek response: {response_text.get("deepseek_response")}
                            You must analyze, compare, and verify the information across all three outputs to ensure accuracy, consistency, and completeness. 
                            Identify aligned findings, conflicting details, missing data, and potential errors.
                            After validating the content, you will produce a final unified analysis that reflects the most reliable insights from the agents’ combined outputs.
                            if any of the agent responses are missing, incomplete, un-accurate, or you never get output 
                            print at the end of your analysis a "Report of Incomplete or Missing Responses".
                            Otherwise Use the following prompt as your guiding structure and analysis al the other agent outout
                            and return your response using this format: {agent_prompt} 
                        ...
                        # TODO: Save the AI result back to the database here!
                        # row.analysis = result
                        # db.session.commit()
